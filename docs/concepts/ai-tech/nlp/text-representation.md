...
文本表示就是把自然语言变成机器能计算的形式。从 One-hot、TF-IDF 到 Word2Vec、BERT Embedding，再到大模型上下文表示，本质都是在回答：怎样让模型知道两段话是否相关、相似或能互相解释。
...
| 方法 | 通俗理解 | 适合场景 |
| --- | --- | --- |
| One-hot | 每个词一个独立位置。 | 教学理解，实际表达能力弱。 |
| TF-IDF | 词越能区分文档，权重越高。 | 关键词检索、传统文本分类。 |
| Word2Vec / GloVe | 把词变成连续向量。 | 语义相似、传统 NLP 特征。 |
| BERT Embedding | 结合上下文理解词句表示。 | 语义检索、匹配、分类。 |
| LLM Context | 在当前上下文中动态理解文本。 | 对话、生成、复杂问答。 |

## 技术判断

如果任务依赖精确词，例如编号、姓名、法规条款，关键词表示仍然有价值。如果任务依赖同义表达，例如“报销酒店限额”和“出差住宿标准”，Embedding 更有优势。

RAG 常常需要混合检索：关键词负责精确命中，向量负责语义召回，Rerank 负责重新排序。
...
1.2 文本表示方法 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
