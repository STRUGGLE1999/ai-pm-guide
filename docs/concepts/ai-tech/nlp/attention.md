...
注意力机制让模型在处理当前内容时，动态关注上下文里更重要的部分。它是 Transformer 和大语言模型的核心直觉。
...
当模型回答“合同里的违约金是多少”时，不应该平均阅读整份合同，而应该重点看“违约责任”“金额”“支付条件”等片段。注意力机制就是让模型学会这种“重点关注”。

## 工程影响

- 上下文越长，注意力计算成本越高。
- 信息放得太乱，模型可能关注错地方。
- 关键信息靠近任务、结构清晰，更容易被模型利用。
- 长上下文不等于长文档问答一定准确。
...
3.2 注意力机制 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
