...
NER 是从文本中识别人名、公司、地点、时间、金额、产品名、订单号等实体。它是合同抽取、舆情分析、简历解析、工单结构化的重要基础。
...
| 实体类型 | 例子 | 应用场景 |
| --- | --- | --- |
| 人名 / 机构 | 张三、某某公司。 | 简历、合同、客户资料。 |
| 时间 / 地点 | 2026 年 6 月、上海。 | 日程、事件抽取。 |
| 金额 / 数量 | 12 万元、3 台。 | 发票、合同、采购。 |
| 业务实体 | 订单号、产品型号、政策条款。 | 工单、知识库、审批。 |

## 难点

NER 的难点不是识别“明显的人名”，而是行业实体、缩写、别名和边界。比如“华东一区销售部”算组织还是区域？“A100”是产品型号还是 GPU？这些都要依赖业务定义。
...
2.2 命名实体识别（NER） 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
