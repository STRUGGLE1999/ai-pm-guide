...
AI 系统越靠近真实业务，越要重视安全、隐私和治理。模型会生成内容，也可能调用工具、读取资料、影响决策，风险不再只是“说错话”。
...
| 风险 | 表现 | 防护 |
| --- | --- | --- |
| Prompt Injection | 外部资料诱导模型忽略规则。 | 上下文隔离、输入检测。 |
| 数据泄露 | 输出或日志包含敏感信息。 | 脱敏、最小留存、权限控制。 |
| 越权调用 | 模型查了不该查的数据。 | 服务端鉴权、参数校验。 |
| 幻觉 | 编造政策、价格、来源。 | 引用、拒答、人工复核。 |
| 误执行 | 发错消息、改错数据。 | 二次确认、审计、回滚。 |
...
安全、隐私与治理 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
