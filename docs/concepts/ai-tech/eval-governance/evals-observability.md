...
AI 功能上线前要评测，上线后要监控，出错时要能追踪。可观测性解决的不是“有没有报错”，而是“为什么模型这次这样回答”。
...
| 层级 | 看什么 |
| --- | --- |
| 离线评测 | 固定题库、标准答案、引用和格式。 |
| 在线监控 | 用户反馈、转人工、失败率、延迟、成本。 |
| 人工抽检 | 复杂业务判断、风险样本、灰度样本。 |
| 回归测试 | 每次改模型、Prompt、知识库后是否退步。 |
...
没有这些记录，Bad Case 只能靠猜。AI 系统要能把错误定位到数据、检索、模型、工具、展示或权限。
...
评测、监控与可观测性 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
