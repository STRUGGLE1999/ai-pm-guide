...
系统层面加速不只优化模型本身，还优化模型外的链路：检索、缓存、网关、队列、异步任务、工具接口和前端体验。
...
用户感知的是完整任务时间，不是单个模型耗时。RAG 检索慢、工具 API 慢、文件解析慢，都会让用户觉得 AI 慢。

所以性能优化要按完整链路排查，不要只盯模型。
...
CH8 系统层面加速 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
