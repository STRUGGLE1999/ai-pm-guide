...
大模型部署框架负责把模型高效地跑起来，并对外提供服务。常见关注点包括显存管理、并发、批处理、缓存、流式输出和多模型路由。
...
框架不是越复杂越好。要看团队能否维护、是否支持现有硬件、并发是否满足业务、是否方便灰度和回滚。

如果产品是实时聊天，首字延迟很重要；如果是批量抽取，吞吐和失败重试更重要。
...
CH3 大模型部署框架解析 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
