...
底层加速关注模型推理内部的计算效率，比如 KV Cache、PagedAttention、算子融合、批处理、内存管理。这些技术决定模型在高并发下能不能稳定服务。
...
| --- | --- |
| KV Cache | 复用已计算的上下文。 |
| PagedAttention | 更高效管理长上下文缓存。 |
| Batch | 合并请求提升吞吐。 |
| 算子融合 | 减少计算和内存开销。 |
| Speculative Decoding | 用小模型辅助加速生成。 |

## 工程影响

底层加速会影响 TTFT、总延迟、吞吐和成本。读者需要把这些指标放进看板，而不是只听“做了优化”。
...
CH7 底层加速 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
