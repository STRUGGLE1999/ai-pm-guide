...
按「成本从低到高、风险从低到高」读，不要一上来微调或私有化。读的时候始终带着同一条产品：**电商售后助手**。用户说「耳机坏了，能换吗，订单号 xxxx」——每一页只加一层能力，看缺口还在哪。

1. [大语言模型基础](/concepts/ai-tech/llm-rag-agent/llm-basics)：它为什么能说、为什么会编。裸模型不得给出换货结论。
2. [提示词工程](/concepts/ai-tech/llm-rag-agent/prompt-engineering)：怎么把任务写成可验收的规格。人设和禁止项进系统提示词，不要教用户写框架。
3. [RAG 与知识库](/concepts/ai-tech/llm-rag-agent/rag-knowledge-base)：内部制度怎么有据可查。订单不要进向量库。
4. [工具调用与工作流](/concepts/ai-tech/llm-rag-agent/tool-calling-workflow)：查单、建单怎么量产；模型提议，后端做主。
5. [Agent、MCP 与多智能体](/concepts/ai-tech/llm-rag-agent/agent-mcp)：客服 v1 先分流到工作流；MCP 降的是接入成本，不是安全成本。
6. [微调该不该做](/concepts/ai-tech/llm-rag-agent/fine-tuning)：前面都不够、且有评测和合法数据时才改权重。制度变更优先改文档。
7. [应用平台与部署选择](/concepts/ai-tech/llm-rag-agent/app-stack-and-deployment)：先分层再选品牌；云 vs 本地用合规和账单说话。
8. 再读对话式 AI、数据标注、多模态等已有专题；方案骨架可对照 [智能客服 Bot](/practice/customer-service-bot)。

每页都按同一套产品问题写：适用/不适用、PRD 字段、对错验收、和研发算法怎么分、面试怎么问。学完应能画一条链路：用户输入 → 提示词 →（检索）→（工具/工作流）→ 输出校验 → 人工确认，并说明每一跳为什么存在、缺了会在售后助手里怎样翻车。

场景怎么选、PRD 怎么写、自动化等级怎么定，见 [产品经理基本功](/concepts/product-basics)；黄金集和 ROI 见 [模型评估与质量](/concepts/model-evaluation)。
