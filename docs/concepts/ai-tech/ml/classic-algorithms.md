...
经典算法不是为了背名词，而是为了帮助你判断“这个问题是否必须用大模型”。很多结构化预测、分类、排序和风控任务，经典机器学习仍然更便宜、更稳定、更容易解释。
...
| 算法 | 通俗理解 | 适合场景 |
| --- | --- | --- |
| 线性回归 | 用特征预测连续数值。 | 销量、成本、时长预测。 |
| 逻辑回归 | 输出某类发生概率。 | 转化、流失、风险判断。 |
| 决策树 | 按条件一步步分叉判断。 | 需要解释的分类任务。 |
| 随机森林 | 多棵树投票。 | 结构化数据分类和回归。 |
| GBDT / XGBoost | 多轮纠错提升效果。 | 风控、广告、推荐特征工程。 |
| SVM | 找类别边界。 | 中小规模分类。 |
| KMeans | 按相似度分组。 | 用户分群、主题聚类。 |
| DBSCAN | 找密集区域和噪声。 | 异常发现、空间聚类。 |
| PCA | 降维压缩信息。 | 可视化、特征压缩。 |
| KNN | 看邻居怎么判断。 | 相似样本、简单分类。 |

## 技术选型要点

如果任务输入是清晰字段，例如年龄、次数、金额、渠道、历史行为，经典模型可能就够用。如果输入是长文本、图片、语音、复杂文档，大模型或深度学习更合适。

不要把“用大模型”当成高级。真正高级的是用合适的能力，以可控成本解决业务问题。
...
机器学习十大算法 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
