...
线性代数和微积分是理解深度学习的底层语言。不必一开始就推导公式，但要知道向量、矩阵、梯度为什么总被提到。
...
| 概念 | AI 里的含义 |
| --- | --- |
| 向量 | 一组数字，可以表示文本、图片、用户或商品。 |
| 矩阵 | 多个向量组成的表，常用于批量计算。 |
| 维度 | 向量有多少个数字，影响表达能力和成本。 |
| 相似度 | 判断两个向量是否接近，常用于检索和推荐。 |

Embedding、向量数据库、推荐系统、本质上都离不开向量和矩阵。

## 微积分对应什么

模型训练时会计算“当前预测离标准答案差多远”，然后沿着让误差变小的方向调整参数。这个方向通常由梯度给出。

你只要记住：梯度帮助模型知道参数该往哪里改；学习率决定每次改多少；如果数据和目标不清楚，再漂亮的优化也可能优化错方向。

## 需要的判断

当研发说“维度太高、显存不够、梯度不稳定、学习率太大、向量相似度低”时，你不必立刻懂公式，但要知道这些问题会影响成本、速度、召回效果和训练稳定性。
...
线性代数与微积分 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
