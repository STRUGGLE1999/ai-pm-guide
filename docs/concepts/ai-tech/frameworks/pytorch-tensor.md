...
Tensor 是 PyTorch 的核心数据结构。它承载输入、输出、模型参数和中间结果。
...
AI 模型处理的不是“原始文字和图片”，而是数字表示。文本会变成 Token 或 Embedding，图片会变成像素矩阵，语音会变成特征序列。这些数字最终都进入 Tensor。
...
- batch size 影响吞吐和延迟。
- 图片尺寸影响速度和效果。
- 数据类型 FP32、FP16、INT8 影响成本和精度。

不需要写 Tensor 操作，但要知道这些设置会影响部署成本和用户体验。
...
PyTorch 张量（Tensor） 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
