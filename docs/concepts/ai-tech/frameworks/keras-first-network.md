...
Keras 适合快速搭建第一个神经网络，帮助团队用较少代码完成数据输入、模型定义、训练、评估和保存。它常用于原型验证和教学，也能进入生产链路。

<figure class="article-figure">
  <img src="/concepts/ai-tech/05-flowchart-ai-quality-loop.png" alt="Keras 模型质量闭环">
  <figcaption>第一个神经网络跑通后，还要继续补评测、监控、版本管理和上线策略。</figcaption>
</figure>
...
<figure class="article-figure">
  <img src="/concepts/ai-tech/01-infographic-ai-pm-map.png" alt="Keras 原型位置">
  <figcaption>Keras 原型要服务于技术判断：任务是否可学、指标是否够好、上线成本是否可控。</figcaption>
</figure>
...
Keras 第一个神经网络 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
