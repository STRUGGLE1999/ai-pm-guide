# PyTorch 与 TensorFlow 导读

训练框架是算法研发把想法变成模型的工具。读者了解框架，不是只为了写代码，而是为了理解模型实验、训练、评估和部署之间的关系。
...
1. [PyTorch 基础](/concepts/ai-tech/frameworks/pytorch-basics)
2. [PyTorch 张量（Tensor）](/concepts/ai-tech/frameworks/pytorch-tensor)
3. [PyTorch 神经网络基础](/concepts/ai-tech/frameworks/pytorch-neural-network)
4. [PyTorch 数据处理与加载](/concepts/ai-tech/frameworks/pytorch-data-loading)
5. [TensorFlow 简介与核心概念](/concepts/ai-tech/frameworks/tensorflow-basics)
6. [Keras 与高级 API](/concepts/ai-tech/frameworks/keras-api)
7. [训练、评估与监控](/concepts/ai-tech/frameworks/training-evaluation-monitoring)

学完这一组，你应该能追问实验结果是否可复现、模型是否可部署、训练和线上处理是否一致。
...
PyTorch 与 TensorFlow 导读 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
