...
Keras 是 TensorFlow 生态中的高级 API，适合快速搭建和训练神经网络。它降低了模型开发门槛，让研发能更快完成实验。
...
高层 API 能提高开发效率，但上线仍然要看推理性能、可监控性、模型版本和服务稳定性。原型跑通不等于生产可用。
...
Keras 与高级 API 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
