...
TensorFlow 环境搭建看似是研发细节，但它会影响项目能不能复现、能不能部署、能不能使用 GPU。私有化和本地部署项目尤其容易卡在版本、驱动和依赖上。

<figure class="article-figure">
  <img src="/concepts/ai-tech/01-infographic-ai-pm-map.png" alt="TensorFlow 环境位置">
  <figcaption>环境搭建属于 AI 工程地基，影响模型实验、训练和部署是否稳定复现。</figcaption>
</figure>
...
| --- | --- |
| Python 版本 | 影响包兼容性。 |
| TensorFlow 版本 | 影响 API、模型格式和性能。 |
| CUDA / cuDNN | 影响 GPU 能否使用。 |
| 操作系统 | 影响部署方式和依赖安装。 |
| 虚拟环境 | 影响团队复现和隔离。 |

<figure class="article-figure">
  <img src="/concepts/ai-tech/05-flowchart-ai-quality-loop.png" alt="环境问题质量闭环">
  <figcaption>环境问题要被记录成可复现清单，避免“只在某个人电脑上能跑”。</figcaption>
</figure>
...
TensorFlow 环境搭建 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
