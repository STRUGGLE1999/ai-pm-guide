...
数据加载决定训练时模型看到什么。PyTorch 里常用 Dataset 和 DataLoader 管理样本、批处理、打乱和并行加载。
...
训练效果差，可能不是模型问题，而是数据加载、切分或增强出了问题。比如训练集和测试集混在一起，指标会虚高；线上输入格式和训练格式不同，效果会骤降。
...
PyTorch 数据处理与加载 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
