...
反向传播是神经网络训练的核心：模型先给出预测，再计算误差，然后把误差反向传回每一层，更新参数。Micrograd 这类教学项目用很小的代码解释自动微分和反向传播。
...
你可以把训练想成不断调旋钮。模型输出错了，就看每个旋钮对错误贡献多少，再按合适方向微调。这个“贡献多少”的信息，就是梯度。

## 为什么仍需了解

反向传播说明模型训练不是“读资料就会”，而是依赖明确目标和大量样本。如果损失函数没有代表业务目标，模型会认真优化一个错误方向。
...
理解反向传播，有助于你把“模型训练”从神秘词变成可讨论的工程过程。
...
反向传播与 Micrograd 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
