...
MLP 是多层感知机，是理解神经网络的基础模型。它由输入层、隐藏层、输出层组成，通过前向传播得到预测，再通过损失和反向传播更新参数。

<figure class="article-figure">
  <img src="/concepts/ai-tech/02-flowchart-llm-call-chain.png" alt="MLP 前向传播链路">
  <figcaption>MLP 能帮助你理解神经网络最小结构：输入、参数、激活、输出和损失。</figcaption>
</figure>

## MLP 为什么重要

许多复杂模型都可以看成在 MLP 基础上扩展出来：CNN 加入局部视觉特征，RNN 处理序列，Transformer 用注意力处理上下文。理解 MLP 后，看复杂架构会更稳。
...
<figure class="article-figure">
  <img src="/concepts/ai-tech/05-flowchart-ai-quality-loop.png" alt="MLP 训练闭环">
  <figcaption>MLP 虽小，也需要训练、评估、错误分析和反馈闭环。</figcaption>
</figure>
...
MLP Readme 翻译 可以拆成输入、处理、输出和指标四层。这样读的时候不会停留在概念名，而是能看到它在系统里接收什么、改变什么、产出什么。
